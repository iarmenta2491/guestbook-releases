...
export default function ReviewScreen({ active }) {
  ...
  return (
    <div className={`screen review-screen${active ? ' active' : ''}`}>
      ...
      {/* ── Main layout: column in portrait, row in landscape ────────────── */}
          ...
          {replayEnabled ? (
            isAudioOnly ? (
              <div className="review-audio-card glass-card-bright">
                ...
                <div className={`review-audio-wave${isPlaying ? ' playing' : ''}`}>
                  {Array.from({ length: 24 }).map((_, i) => (
                    <div key={i} className="review-audio-bar" style={{ animationDelay: `${i * 0.06}s` }} />
                  ))}
                </div>
              ...
              >
                ...
                >
                  {!isPlaying && <div className="review-play-icon">▶</div>}
                </button>
            ...
            <div className="review-static-card glass-card-bright">
              <div className="review-static-icon">{isAudioOnly ? '🎙️' : '🎬'}</div>
